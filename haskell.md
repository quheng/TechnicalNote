...
在 Haskell 98 中, 待注册的数据类型(例子中的 Bool)不能是类型参数如 String([Char]). 若要使用的话在文件的开头加上`{-# LANGUAGE TypeSynonymInstances #-}`. 若同时注册多个相同的函数, 会发生 overlapping, 可以使用OverLappingInstances 解决.
